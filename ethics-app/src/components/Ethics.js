...
const Ethics = () => {
  ...
  const [sectionE, setSectionE] = useState(null);
  ...
  // Fetch Section E (SupervisorForm) from Firebase
  useEffect(() => {
    const fetchSectionE = async () => {
      if (!selectedApplicantId) return;
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setSectionE(data.sectionE || null);
        } else {
          setSectionE(null);
        }
      } catch (err) {
        console.error("Error fetching Section E:", err);
        setError("Failed to fetch supervisor form data.");
      }
    };
        ...
        <div className="applicant-review-container">
          ...
          <div className="progress-container">
            ...
            <div className="step-indicators">
              ...
              <span className={currentStep >= 5 ? "active" : ""}>Section E</span>
            </div>
          </div>
          ...
          {/* STEP 5: SECTION E (Supervisor Form) */}
          {currentStep === 5 && (
            <div className="applicant-details section">
              <h3>SECTION E: Supervisor Form</h3>
              
              {sectionE ? (
                <div className="section-e-content">
                  <div className="fields-grid">
                    <ClickableField label="Supervisor Name" value={sectionE.supervisorName} keyName="supervisorName" />
                    <ClickableField label="Persal Number" value={sectionE.persalNumber} keyName="persalNumber" />
                    <ClickableField label="Applicant Name" value={sectionE.applicantName} keyName="applicantName" />
                    
                    {/* Recommendation */}
                    <div className="clickable-field">
                      <strong>Recommendation:</strong>
                      <span className="field-value">
                        {sectionE.supervisorRecommendation === '5.1' && 'Recommended'}
                        {sectionE.supervisorRecommendation === '5.2' && 'Recommended with conditions'}
                        {sectionE.supervisorRecommendation === '5.3' && 'Not recommended'}
                      </span>
                    </div>
                    
                    {sectionE.supervisorRecommendation === '5.2' && (
                      <ClickableField label="Conditions" value={sectionE.supervisorConditionsText} keyName="supervisorConditionsText" />
                    )}
                    
                    {sectionE.supervisorRecommendation === '5.3' && (
                      <ClickableField label="Reason" value={sectionE.supervisorReasonText} keyName="supervisorReasonText" />
                    )}
                    
                    {/* Checklist items */}
                    <div className="checked-items">
                      <h4>Checklist:</h4>
                      <div className={`checked-item ${savedFields.includes('performanceContract') ? "saved" : ""}`}
                          onClick={() => saveField('performanceContract', sectionE.performanceContract)}>
                        <span>Performance contract reviewed: {sectionE.performanceContract ? "Yes" : "No"}</span>
                        <button className="btn-sm">
                          {savedFields.includes('performanceContract') ? "✓ Reviewed" : "Mark as Reviewed"}
                        </button>
                      </div>
                      
                      <div className={`checked-item ${savedFields.includes('hoursReviewed') ? "saved" : ""}`}
                          onClick={() => saveField('hoursReviewed', sectionE.hoursReviewed)}>
                        <span>Hours of work reviewed: {sectionE.hoursReviewed ? "Yes" : "No"}</span>
                        <button className="btn-sm">
                          {savedFields.includes('hoursReviewed') ? "✓ Reviewed" : "Mark as Reviewed"}
                        </button>
                      </div>
                      
                      <div className={`checked-item ${savedFields.includes('noConflicts') ? "saved" : ""}`}
                          onClick={() => saveField('noConflicts', sectionE.noConflicts)}>
                        <span>No conflicts of interest: {sectionE.noConflicts ? "Yes" : "No"}</span>
                        <button className="btn-sm">
                          {savedFields.includes('noConflicts') ? "✓ Reviewed" : "Mark as Reviewed"}
                        </button>
                      </div>
                      
                      <div className={`checked-item ${savedFields.includes('concernsAddressed') ? "saved" : ""}`}
                          onClick={() => saveField('concernsAddressed', sectionE.concernsAddressed)}>
                        <span>Concerns have been addressed: {sectionE.concernsAddressed ? "Yes" : "No"}</span>
                        <button className="btn-sm">
                          {savedFields.includes('concernsAddressed') ? "✓ Reviewed" : "Mark as Reviewed"}
                        </button>
                      </div>
                    </div>
                    
                    <ClickableField label="Signature" value={sectionE.signature} keyName="signature" />
                    <ClickableField label="Designation" value={sectionE.designation} keyName="designation" />
                    <ClickableField label="Date" value={sectionE.signatureDate} keyName="signatureDate" />
                  </div>
                </div>
              ) : (
                <div className="no-section-e">
                  <p>No supervisor form has been submitted for this applicant yet.</p>
                  <button 
              ...
              <div className="navigation-buttons">
                ...
                <button className="btn btn-primary" onClick={() => navigate("/ethicform")}>
                  Fill The Form
                </button>
              </div>
            </div>
